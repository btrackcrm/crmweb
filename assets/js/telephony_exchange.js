	...
	function refreshEmails(showLoading){
				...
				$.ajax({
					...
					success: function(response) {
						if (!response.error){
							...
							swal(
								'Error',
								'Error retrieving inbox mail count ' + response.message,
								'error'
							);
						}						
					}
        ...
        function viewEmail(index) {
			...
			$.ajax({
                type: "POST",
                url:  "telephony/getexchangemail",
                data: postData,
				dataType: "json",
                success: function(responseMail) {
					console.log(responseMail);
					var details = responseMail.emaildetails;
					email.message = details.message;
					email.attachments = details.attachments;
					currentMail = email;
					var dbMail = responseMail.email;
					...
					for (var i = 0; i < files.length; i++) {
						var curFile = files[i];
						var attachmentLink = "/Uploads/EmailAttachments/" + encodeURI(curFile.name);
