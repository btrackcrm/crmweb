...
<body>
		...
		<div class="popupContainer" id="newMailboxPopup" hidden>
			<div class="panel panel-inverse" id="newMailboxDIV">
                ...
                <div class="panel-body">
					<form id="newMailboxForm" action="<?= BASE_URL . "webmail/exchangefoldercreate" ?>" method="post" class="form-horizontal">
	...
	<script>
		...
		$(document).ready(function() {
			perPage = Cookies.get("exchange_page", 15);
			if (perPage == undefined){
				perPage = 15;
				Cookies.set("exchange_page", 15, { expires: 365 });
			}
			getFolders();
			...
			$("#pageList li").click(function(){
				...
				Cookies.set('exchange_page', perPage, { expires: 365 });
				refreshEmails();
			});
			...
			$('#newMailboxForm').on('submit', function(e) { //use on if jQuery 1.7+
                e.preventDefault();
                    $.ajax({
                    	type: "POST",
                    	url: "<?= BASE_URL ?>" + "webmail/exchangefoldercreate",
			...
			$('#sendEmailForm').on('submit', function(e) { //use on if jQuery 1.7+
                ...
                if (recipients.length == 0){
					...
					if (currentMail != null){
						postData.append('reply_mail_id', currentMail.mail_id);
						postData.append('reply_mail_change_key', currentMail.change_key);
					}
					console.log(postData);
                    $.ajax({
                    	type: "POST",
                    	url: "<?= BASE_URL ?>" + "email/exchangesend",
						processData: false,
						contentType: false,
						cache: false,
                    	data: postData,
		...
		function getFolders(){
			$.ajax({
                type: "POST",
                url: "<?= BASE_URL ?>" + "webmail/exchangefolders",
                data: null,
                dataType: "json",
                success: function(response) {
					console.log(response);
					var folders = response.folders;
					folderArray = response.folders;
					var moveListContent = "";
					$("#folderList").html("");
					var inboxIdx = null;
		...
		function moveSelectedEmail(folderIndex, emailsToMove){
			...
			var postData = { "mail_ids": toMove, "folder_id": moveTo.id, "folder_change_key": moveTo.change_key };
			$.ajax({
				type: "POST",
				url: "<?= BASE_URL ?>" + "webmail/exchangemove",
				data: postData,
				success: function(response) {
					console.log(response);
					if (!response.error){
		...
		function getEmails(){
				if (loadedPages.indexOf(currentPage) == -1){ //we haven't loaded this page yet, so load it
					...
					$.ajax({
						type: "POST",
						url: "<?= BASE_URL ?>" + "webmail/exchangelist",
						data: postData,
						dataType: "json",
						success: function(response) {
		...
		function filterEmails(){
			...
			var postData = { "query": searchQ, "folder_id": currentFolder.id };
					...
					$.ajax({
						type: "POST",
						url: "<?= BASE_URL ?>" + "webmail/exchangesearch",
						data: postData,
						dataType: "json",
						success: function(response) {
							console.log(response);
		...
		function toggleStarred(item, index){
            ...
            if (isStarred){
				...
				$.ajax({
					type: "POST",
					url: "<?= BASE_URL ?>" + "webmail/exchangestar",
					data: postData,
					success: function(response) {
						console.log(response);
						email.flagged = 0;
		...
		function viewEmail(index) {
			...
			$.ajax({
                type: "POST",
                url: "<?= BASE_URL ?>" + "webmail/getexchangemail",
                data: postData,
				dataType: "json",
                success: function(responseMail) {
					email.message = responseMail.message;
					email.attachments = responseMail.attachments;
					currentMail = email;
					...
					for (var i = 0; i < files.length; i++) {
						var curFile = files[i];
						var attachmentLink = "<?= UPLOADS_URL ?>" + "EmailAttachments/" + encodeURI(curFile.name);
						if (curFile.cid != "" && curFile.inline){
							var regex = new RegExp('"cid:' + curFile.cid + '"', "g");
		...
		function markEmailAsRead(){
            var postData = { "mail_id": currentMail.mail_id, "change_key": currentMail.change_key };
            $.ajax({
                type: "POST",
                url: "<?= BASE_URL ?>" + "webmail/exchangeread",
                data: postData,
				dataType: "json",
                success: function(response) {
					if (!response.error){
		...
		function deleteEmails(emailsToDelete){
			...
			var postData = { "mail_ids": toDelete, "folder": currentFolder.id };
			$.ajax({
				type: "POST",
				url: "<?= BASE_URL ?>" + "webmail/exchangedelete",
				data: postData,
				success: function(response) {
					if (response == ""){
						swal(
		...
		function deleteEmail(item, index){
			swal({
                ...
                confirmButtonText: 'Delete e-mail'
                    ...
                    var postData = { "mail_ids": currentMail.mail_id, "folder": currentFolder.id };
					$.ajax({
						type: "POST",
						url: "<?= BASE_URL ?>" + "webmail/exchangedelete",
						data: postData,
