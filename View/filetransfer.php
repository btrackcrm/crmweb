...
<body>
        ...
        <div id="content" class="content content-full-width">
			<ul class="nav nav-tabs">
				<li class="nav-items active">
					<a href="#files-tab" data-toggle="tab" class="nav-link active show">
			...
			<div class="tab-content">
				<div class="tab-pane fade in active" id="files-tab">
					<div class="full-height-background">
						<div class="upload-window p-15">
							<h4>File upload</h4>
							<p>Send your files safely and in a way compliant with GDPR laws<p>
							<form id="myDropzone" class="form-horizontal dropzone" method="POST" enctype="multipart/form-data" action="file/transfer">
								<div class="form-group">
									<div class="col-md-12">
										<input id="email_to" type="email" class="form-control" name="email_to" placeholder="E-mail to" />
									</div>
								</div>
								<div class="form-group">
									<div class="col-md-12">
										<input id="email_from" type="email" class="form-control" name="email_from" placeholder="Your e-mail" />
									</div>
								</div>
								<div class="form-group">
									<div class="col-md-12">
										<input id="phone_number" type="text" class="form-control" name="phone" placeholder="Send SMS with password to" />
									</div>
								</div>
								<div class="form-group">
									<div class="col-md-12">
										<textarea id="email_message" class="form-control" name="message" rows="4" placeholder="Message" ></textarea>
									</div>
								</div>
								<button id="transferBtn" class="btn btn-success btn-block"><i class="fas fa-exchange-alt m-r-5"></i>Send files</button>
							</form>
						</div>
					</div>
				</div>
						...
						<div class="col-md-12">
							<div class="panel panel-primary">
									<div class="panel-heading">
										...
										<h4 class="panel-title">List of transfers</h4>
									</div>
									<div class="panel-body">
										<div class="dataTables_wrapper form-inline dt-bootstrap no-footer">
											<table id="fileTransferTable" class="table table-striped">
												<thead>
													<tr>
														<th>
															Transferred by
    ...
    <script>
        ...
        $(document).ready(function() {
			...
			$("#phone_number").intlTelInput({ initialCountry: "auto",
			hiddenInput: "phone",
			...
			Dropzone.options.myDropzone= {
				url: "<?= BASE_URL ?>" + 'file/transfer',
				dictDefaultMessage: "<b>Drop</b> files here or <b>click</b> to upload",
				...
				init: function() {
					...
					this.on("sendingmultiple", function(data, xhr, formData) {
						formData.append("phonenr", $("#phone_number").intlTelInput("getNumber"));
					});
		...
		function getFileTransfers(){
			$.ajax({
                type: "POST",
                url: "<?= BASE_URL ?>" + "filetransfer/getall",
                data: null,
                dataType: "json",
                success: function(filetransfers) {
					if (dTable != null){
                        dTable.clear().rows.add(filetransfers).draw();
                    }else{
						dTable = $('#fileTransferTable').DataTable({
							"aaData": filetransfers,
							"columns": [
								{ "data": "employee_id" },
								{ "data": "customer_id" },
								{ "data": "email_to" },
								...
								},
								{
									...
									"render": function ( data, type, row ) {
										if (data == null){
											...
											return '<a href="customerdetails?customer_id=' + data + '" target="_blank">' + row.customer_name + '</a>';
										}
									},
									"targets": 1
								},
