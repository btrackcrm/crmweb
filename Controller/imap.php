...
class Mailbox {
	...
	/**
	 * Remove the flag \Flagged from a mail.
	 * @param $mailId
	 */
	public function markMailAsUnimportant($mailId){
		$this->clearFlag([$mailId], '\\Flagged');
	}
	/**
	 ...
	 * @return array
	 */
	public function getMailsInfo($from, $to) {
		$mails = imap_fetch_overview($this->getImapStream(), $to . ":" . $from, 0);
	...
	/**
	 * Get an X amount of mails from current mailbox,
	 * returns an array of mails,
	 * 
	 *
	 * @return array
	 */
	public function getPageMails($from_to, $options) {
		$mails = $this->imap('fetch_overview', $from_to, $options);
		return $mails;
	}
	/**
	 ...
	 * @return mixed
	 */
	public function getRawMail($msgId, $markAsSeen = false) {
		$options = FT_UID;
		if(!$markAsSeen) {
			$options |= FT_PEEK;
		}
		$mailStructure = $this->imap('fetchstructure', [$msgId, FT_UID]);
		$data = $this->imap('fetchbody', [$msgId, '1', $options]);
	...
	public function getMail($mailId, $markAsSeen = true) {
		...
		$mailOverview = $this->imap('fetch_overview', [$mailId, FT_UID]);
		...
		$mail->seen = $mailOverview[0]->seen;
		$mail->answered = $mailOverview[0]->answered;
		$mail->flagged = $mailOverview[0]->flagged;
		if (isset($mailOverview[0]->in_reply_to)){
			$mail->in_reply_to = $mailOverview[0]->in_reply_to;
		}
		$mail->msgID = $mailOverview[0]->udate . $mailOverview[0]->uid;
		return $mail;
	}
	...
	public function getHeaders($mailId){
		...
		$mail->in_reply_to = $mailOverview[0]->in_reply_to;
		$mail->answered = $mailOverview[0]->answered;
		$mail->uid = $mailOverview[0]->message_id;
		return $mail;
	}
	
	protected function initMailPart(IncomingMail $mail, $partStructure, $partNum, $markAsSeen = true) {
		
		if (!$this->attachmentsEnabled &&($partStructure->type !== TYPEMULTIPART && ($partStructure->type !== TYPETEXT || !in_array(strtolower($partStructure->subtype), ['plain','html'])))){ // skip all but plain and html when attachments are not required
			return false;
		}
